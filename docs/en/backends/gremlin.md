...
# First pass: every node arrives before any relationship does, so collect
# vertices before wiring anything up.
...
# Second pass: the endpoints are all in `vertices` by now.
